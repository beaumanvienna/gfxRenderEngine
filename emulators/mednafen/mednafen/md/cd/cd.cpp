...
static MDFN_FASTCALL void MDCD_MainWrite8(uint32 A, uint8 V)
{
 ...
 else if(A >= 0xa12000 && A <= 0xa120ff)
 {
  switch(A & 0xFF)
  {
   ...
   case 0x1D:
   case 0x1E:
   case 0x1F: WRITE_BYTE_MSB(COMM_CMD, A & 0xF, V);
		       break;
   case 0x20:
   case 0x21:
   case 0x22:
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: break; // Unused on main cpu writes
...
static MDFN_FASTCALL void MDCD_MainWrite16(uint32 A, uint16 V)
{
 ...
 else if(A >= 0xa12000 && A <= 0xa120ff)
 {
  switch(A & 0xFF)
  {
   ...
   case 0x1D:
   case 0x1E:
   case 0x1F: WRITE_WORD_MSB(COMM_CMD, A & 0xF, V);
		       break;
   case 0x20:
   case 0x21:
   case 0x22:
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: // Unused on main CPU writes
		       break;
  }
 }
}
...
static MDFN_FASTCALL uint8 MDCD_MainRead8(uint32 A)
{
 ...
 else if(A >= 0xa12000 && A <= 0xa120ff) 
 {
  ...
  switch(A & 0xFF)
  {
   ...
   case 0x1D:
   case 0x1E:
   case 0x1F: return(READ_BYTE_MSB(COMM_CMD, (A & 0xF)));
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: return(READ_BYTE_MSB(COMM_STATUS, (A & 0xF)));
  }
 }
...
static MDFN_FASTCALL uint16 MDCD_MainRead16(uint32 A)
{
 ...
 else if(A >= 0xa12000 && A <= 0xa120ff)
 {
  ...
  switch(A & 0xFF)
  {
   ...
   case 0x1D:
   case 0x1E:
   case 0x1F: return(READ_WORD_MSB(COMM_CMD, (A & 0xF)));
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: return(READ_WORD_MSB(COMM_STATUS, (A & 0xF)));
  }
 }
...
static MDFN_FASTCALL void MDCD_SubWrite8(uint32 A, uint8 V)
{
 ...
 else if(A >= 0xFF8000 && A <= 0xFF81FF) // Registers(as if everything else isn't ;))
 {
  ...
  switch(A & 0x1FF)
  {
   ...
   case 0x1D:
   case 0x1E:
   case 0x1F: break; // Unused on sub cpu writes
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: WRITE_BYTE_MSB(COMM_STATUS, (A & 0xF), V);
		       break;
...
static MDFN_FASTCALL void MDCD_SubWrite16(uint32 A, uint16 V)
{
 ...
 else if(A >= 0xFF8000 && A <= 0xFF81FF) // Registers(as if everything else isn't ;))
 {
  ...
  switch(A & 0x1FF)
  {
   ...
   case 0x2D:
   case 0x2E:
   case 0x2F: WRITE_WORD_MSB(COMM_STATUS, (A & 0xF), V);
                       break;
...
static int32 CheckValidTrack(CDInterface* cdiface, uint8 *sector_buffer)
{
 CDUtility::TOC toc;
 bool DTFound = 0;
 int32 track;
 ...
 {
  if(cdiface->ReadSectors(sector_buffer, toc.tracks[track].lba, 1))
  {
...
void MDCD_Load(std::vector<CDInterface*> *CDInterfaces, md_game_info *ginfo)
{
 try
 {
  uint8 sector_buffer[2048];
